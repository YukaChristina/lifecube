...
直近のリファクタリングでは、カメラ画面に集まっている以下の責務を小さく分離することを優先します。
...
カメラを開く
  ...
  -> 前後カメラ撮影の場合:
       ...
       内側カメラへ切り替え
       内側カメラの準備完了後に撮影
       Skia で縦向き9:16または横向き16:9のローカル画像合成
       アプリ内保存領域に外側、内側、合成後の3枚を保存
       合成後写真だけ端末の写真アプリへ保存
  -> 背面カメラのみ撮影の場合:
       背面カメラで撮影
       アプリ内保存領域に背面写真を成果物画像として保存
       背面写真を端末の写真アプリへ保存
  -> SQLite に写真セット情報を保存
  -> アルバムに表示
```
...
- 写真セット管理用の SQLite
- 端末の写真アプリには成果物画像だけを保存
- 背面カメラのみ撮影では、背面写真を成果物画像として LifeCube アルバムと端末の写真アプリに保存
- 写真セットには撮影モード、画面向き、成果物画像サイズを保存
